...
/*   Updated: 2024/01/23 14:15:34 by psanger          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	y_coordiante(int x, int y, t_data *data)
{
	...
	data->temp = sin(data->r1 * PI / 180) * data->distance * data->matrix[y][x];
	if (len == 0)
		return (HEIGHT / 2 - data->temp + data->move_y);
	y2 = sin(((alpha + data->rotation) * PI) / 180) * len + HEIGHT / 2;
	y2 = HEIGHT / 2 + (y2 - HEIGHT / 2) * cos(data->r1 * PI / 180) - data->temp;
